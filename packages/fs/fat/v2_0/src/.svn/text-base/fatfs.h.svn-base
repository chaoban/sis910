...
typedef struct long_dentry_s 
{
    cyg_uint8    id;             /* sequence number for slot */
    char         name0_4[10];    /* first 5 characters in name */
    cyg_uint8    attr;           /* attribute byte */
    cyg_uint8    reserved;       /* always 0 */
    cyg_uint8    alias_checksum; /* checksum for 8.3 alias */
    char         name5_10[12];   /* 6 more characters in name */
    cyg_uint16   start;          /* starting cluster number, 0 in long slots */
...
#define ATTR_LNAME     (ATTR_RO | ATTR_HIDDEN | ATTR_SYS | ATTR_VOLUME)
...
int read_next_raw_dentry(fatfs_disk_t *disk,fatfs_data_pos_t *pos,fat_raw_dir_entry_t *dentry);
#endif
// --------------------------------------------------------------------------
