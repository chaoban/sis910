...
//#define FUNC_DBG_PTR     diag_printf("[INFO] %s()\n", __FUNCTION__)
#define FUNC_DBG_PTR     CYG_EMPTY_STATEMENT
...
static int fatfs_listdir(cyg_mtab_entry *mte, cyg_dir dir, const char *name, struct vfat_descriptor *ventry);
...
find_direntry(fatfs_dirsearch_t *ds)
{
    ...
    {  
        ...
        err = fatfs_read_dir_entry(ds->disk, ds, &pos, &dentry);
        if (err != ENOERR)
        ...
        if ('\0' == ds->long_name[ds->namelen] &&
               0 == strncasecmp(ds->long_name, ds->name, ds->namelen))
        {
            // Dir entry found - allocate new node and return

            //read the 8.3 raw entry
            err = fatfs_read_dir_entry(ds->disk, ds, &pos, &dentry);
            ds->node = fatfs_node_alloc(ds->disk, &dentry);
            if (NULL == ds->node)
                return EMFILE;
...
//diag_printf("%s:%d\n", __FUNCTION__, __LINE__);    
    ...
    if (err != ENOERR)
        return err;
#ifdef ALIGN_USE_FUNC_PTR    
    dev_t = (cyg_devtab_entry_t *)dev_h;
#endif
...
#ifdef SUPPORT_ALIGN_MOUNT
    disk->bcache_mem = (cyg_uint8 *)malloc_align(CYGNUM_FS_FAT_BLOCK_CACHE_MEMSIZE, mte->align); // align n bytes
#else
#ifdef ALIGN_USE_FUNC_PTR
    disk->bcache_mem = (cyg_uint8 *)dev_t->func_ptr(CYGNUM_FS_FAT_BLOCK_CACHE_MEMSIZE);
#else
    disk->bcache_mem = (cyg_uint8 *)malloc(CYGNUM_FS_FAT_BLOCK_CACHE_MEMSIZE);
#endif
#endif
//    diag_printf("address %d\n",disk->bcache_mem);
            ...
            cyg_dir        *dir_out)
{
    ...
    if (dir_out != NULL)
    {
        ...
        fatfs_dirsearch_t ds1 = {0} ,ds2 = {0},*ds_out = NULL;
        int               err;
...
fatfs_fo_dirread(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
    ...
    ds.dir->dentry = fd->node->dentry;

    if (len < sizeof(struct dirent))
        return EINVAL;

    err = fatfs_read_dir_entry(disk, &ds, &fd->pos, &dentry);
...
//--------------------------------------------------------------------------
#ifdef CYGPKG_FS_VFAT_NAME
static int lname_find(fatfs_dirsearch_t *ds, struct vfat_descriptor *ventry)
{
    int err;
  	fatfs_dir_entry_t *dir_entry = NULL, dentry;
    fatfs_data_pos_t    pos;
    ...
    for(;;)
    {
        ...
        if (ds->last) // entry  found
        {

          fatfs_initpos(ds->disk, &ds->node->dentry, &pos);
          if (FATFS_FAT32 == ds->disk->fat_type && 0 == ds->node->dentry.disk_pos.cluster)
              pos.cluster = ds->disk->fat_root_dir_cluster;
          
          // find the dir entry of the last name of gived path 
          while (true)
          {
            read_next_raw_dentry(ds->disk, &pos, &raw_dentry);

            //found something not deleted or free entry
            if ( (0xE5 != (cyg_uint8)raw_dentry.name[0]) && (0x00 != (cyg_uint8)raw_dentry.name[0])  )
            { 
              dentry.disk_pos = pos;
              break;
            }

            //this is the last entry in the dir, means no entry in dir
            if( 0x00 == (cyg_uint8)raw_dentry.name[0] )
               return 0;

            pos.cluster_pos += 0x20; // add the length of dentry
          }
          ...
          if (err != ENOERR)
        	...
        	dir_entry = &dentry;
          err = fatfs_parser(dir_entry, ds->disk, ventry);
            return err;
        }
...
// mte = mount table entry 
//dir = current dir
//name = lookup directory name
static int
fatfs_listdir(cyg_mtab_entry *mte, cyg_dir dir,const char *name, struct vfat_descriptor *ventry)
{
    fatfs_disk_t       *disk = (fatfs_disk_t *) mte->data;
    fatfs_dirsearch_t   ds = {0};
    int                 err;

    //init  de struct depend on mount table 
    init_dirsearch(&ds, disk, (fatfs_node_t *) dir, name); 
    err = lname_find(&ds,ventry);
    return err;
}
#else
static int fatfs_listdir(cyg_mtab_entry *mte, cyg_dir dir,const char *name, struct vfat_descriptor *ventry)
