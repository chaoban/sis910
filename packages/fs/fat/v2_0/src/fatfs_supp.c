...
#define DENTRY_IS_LFN(_dentry_) \
	(0x0F == (cyg_uint8)((_dentry_)->attr))
...
#define TRIM_RIGHT_STRING(_continue_, _str_, _str_len_) \
	_continue_ = 1;                                     \
	while(_str_len_ > 0 && _continue_ )                 \
    {                                                   \
        if(_str_[_str_len_ -1] == ' ')    _str_len_--;  \
        else    _continue_ = 0;                         \
    }                                                   \
    _str_[_str_len_] = '\0';                            \
// -------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
//unicode to ASCII
void unicode2ascii(char *uni_name, char *dest , int cnt )
{
  int i,j;
    for(i=0,j=0; i < cnt; i=i+2,j++)
    {
      if( (*(uni_name+i) & 0xFF) == 0xFF  )
        {
          *(dest+j) = '\0';
          break;
        }
                ...
                fat_raw_dir_entry_t *dentry)
{
    ...
    cyg_uint32 len;
    int err;
#ifdef CYGPKG_FS_VFAT_NAME
    long_dentry_t *long_de = NULL;
#endif
...
#ifdef CYGPKG_FS_VFAT_NAME
    GET_BYTE(data, dentry->attr, 0x0B); //get attribute

if ( (dentry->attr & 0x0F) == 0x0F ) //compare attribute for vfat entry
    {
    long_de = (long_dentry_t *)dentry;
    GET_BYTE(data,  long_de->id,             0x00);
    GET_BYTES(data, long_de->name0_4,    10, 0x01);
    GET_BYTE(data,  long_de->attr,           0x0B);
    GET_BYTE(data,  long_de->reserved,       0x0C);
    GET_BYTE(data,  long_de->alias_checksum, 0x0D);
    GET_BYTES(data, long_de->name5_10,    12,0x0E);
    GET_BYTE(data,  long_de->start,          0x1A);
    GET_BYTES(data, long_de->name11_12,   4, 0x1C);
  	dentry=(fat_raw_dir_entry_t *)long_de;
    }
...
#ifdef CYGPKG_FS_VFAT_NAME
static int write_raw_LFN( fatfs_disk_t *disk, fatfs_data_pos_t *pos, long_dentry_t *dentry )
{
    cyg_uint8  data[DENTRY_SIZE];
    cyg_uint32 len;
    int err;
   
    SET_BYTES(data, dentry->name0_4,     10, 0x01);
    SET_BYTES(data, dentry->name5_10,    12, 0x0E);
    SET_BYTES(data, dentry->name11_12,    4, 0x1C);
    SET_BYTE(data,  dentry->id,              0x00);
    SET_BYTE(data,  dentry->attr,            0x0B);
    SET_BYTE(data,  dentry->reserved,        0x0C);
    SET_BYTE(data,  dentry->alias_checksum,  0x0D);    
    SET_BYTE(data,  dentry->start,           0x1A);
...
#ifdef CYGPKG_FS_VFAT_NAME
static void
init_LFN_entry( long_dentry_t *dentry, const char *name )
{
    const char  *ptr = name;
    int     i;

    memset(dentry->name0_4, 00, 10);
    for( i=0 ; i<10 && *ptr != '\0' ; i=i+2,ptr++ )
      dentry->name0_4[i] = *ptr;
    
    memset(dentry->name5_10, 00, 12);
    for( i = 0 ; i<12 && ptr != '\0' ; i=i+2,ptr++ )
      dentry->name5_10[i] = *ptr;

    memset(dentry->name11_12, 00, 4);
    for( i = 0 ; i<4 && ptr[i] != '\0' ; i=i+2,ptr++ )
      dentry->name11_12[i] = *ptr;          
      
#ifdef CYGCFG_FS_FAT_USE_ATTRIBUTES
        dentry->attr = 0x0F;
#endif // CYGCFG_FS_FAT_USE_ATTRIBUTES
    
    dentry->reserved = 0x00;
    dentry->alias_checksum = 0x00; //fixme
    dentry->start = 0x00;
}
#endif
...
//diag_printf("%s:%d\n", __FUNCTION__, __LINE__);
    ...
    {
//        diag_dump_buf_with_offset(&boot_rec, sizeof(boot_rec), &boot_rec);
        return EINVAL;
    }
//diag_printf("%s:%d\n", __FUNCTION__, __LINE__);
                     ...
                     fatfs_dir_entry_t *dentry)
{
    ...
    long_dentry_t       *long_de = NULL;
#endif
    fatfs_dir_entry_t   *dir = &ds->dir->dentry;
...
#ifdef CYGPKG_FS_VFAT_NAME
	//filled long name into ds->long_name
	long_de = (long_dentry_t *)(&raw_dentry);

    if( DENTRY_IS_LFN(long_de) )
      {
        seqnum = (int)(long_de->id & ~0x40);
        unicode2ascii(long_de->name0_4  , &ds->long_name[(seqnum-1)*13]    ,10 );
        unicode2ascii(long_de->name5_10 , &ds->long_name[(seqnum-1)*13+5]  ,12 );
        unicode2ascii(long_de->name11_12, &ds->long_name[(seqnum-1)*13+11] , 4 );	
      }
#endif
...
#ifdef CYGPKG_FS_VFAT_NAME    
    long_dentry_t       *LFN_entry = NULL;
    fat_raw_dir_entry_t  pre_dentry;
    fatfs_data_pos_t     dentry_pos = file->disk_pos;
    do
      {
         dentry_pos.cluster_pos -= DENTRY_SIZE;
         read_raw_dentry(disk, &dentry_pos, &pre_dentry);
         LFN_entry = (long_dentry_t *)(&pre_dentry);
         if( LFN_entry->attr == 0x0F )
           {
             LFN_entry->id = 0xE5;
             write_raw_dentry(disk, &dentry_pos, &pre_dentry);
           }
         else
           break;
      }while( true );
    cyg_blib_flush(&disk->blib);
#endif
    return err;
} 
                  ...
                  fatfs_dir_entry_t *dentry)
{
    ...
    if( m_namelen > 8 )
      {
        int frg_cnt,frg_len,remainder;
        long_dentry_t  LFN_entry;
        const char *name_ptr;
        fat_raw_dir_entry_t raw_dentry;

       if( disk->fat_type == FATFS_FAT32 && pos.cluster == 0 )
          pos.cluster = disk->fat_root_dir_cluster;

       while (true) //find the last entry in dir
       {
                     //get the last entry in dir
          if (DENTRY_IS_ZERO(&raw_dentry))
              break;
          ...
          pos.cluster_pos += DENTRY_SIZE;
            ...
            // Change cluster if needed
          if (pos.cluster_pos >= disk->cluster_size)
              err = get_next_cluster(disk, &pos, CO_EXTEND | CO_ERASE_NEW);
              
          err = read_raw_dentry(disk, &pos, &raw_dentry);
          if (err != ENOERR)
              diag_printf("error: in fatfs_create_file function\n");
       }

        frg_len = namelen;
        frg_cnt = frg_len / 13;
        remainder = frg_len % 13;
        
        if( 0 != remainder )
          name_ptr = name+13*frg_cnt;
        else
          {
            frg_cnt--;
            name_ptr = name+13*frg_cnt;
        ...
        init_LFN_entry( &LFN_entry, name_ptr );
        LFN_entry.id = 64+frg_cnt+1;  //mask 0x40 with id field

        err = write_raw_LFN(disk, &pos, &LFN_entry);
        if (err != ENOERR)
          return err;

        pos.cluster_pos += DENTRY_SIZE;
        
        for( frg_cnt-- ; frg_cnt >= 0 ; frg_cnt-- )
          {
            name_ptr = name+13*frg_cnt;
            init_LFN_entry( &LFN_entry, name_ptr );
            LFN_entry.id = frg_cnt+1;
            err = write_raw_LFN(disk, &pos, &LFN_entry);
...
#ifdef CYGPKG_FS_VFAT_NAME
    //process ext file name

    if( is_LFN )
      {
         const char *name_ptr1;
         int ext_len, x=0, y=0;

         for( name_ptr1 = name; name_ptr1 != '\0'; name_ptr1++,x++ )
           if( *name_ptr1 == '.' )
              y = x+1;
        
        name_ptr1 = name+y;
        ext_len = strlen(name_ptr1);
        if( ext_len <= 3 )
           for( x=9 ;name_ptr1 != '\0'; name_ptr1++, x++ )
              dentry->filename[x] = *name_ptr1;
      }
#endif
    ...
    if( namelen > 8 )
      {
       ...
       while (true) //find the last entry in dir
       {
          ...
          err = read_raw_dentry(disk, &pos, &raw_dentry);
          if (err != ENOERR)
              diag_printf("error: in fatfs_create_dir function\n");
       }
                  ...
                  int                namelen)
{
    ...
    int                 err;
#ifdef CYGPKG_FS_VFAT_NAME
    fatfs_data_pos_t    old_pos;
    int                 is_long=0;
    long_dentry_t      *LFN_ptr = NULL;
    fat_raw_dir_entry_t pre_dentry;
#endif
    ...
    old_pos.cluster_pos -= DENTRY_SIZE;
       
    err = read_raw_dentry(disk, &old_pos, &pre_dentry);
    if (err != ENOERR)
       diag_printf("error: no previous dir entry\n");
          
    LFN_ptr = (long_dentry_t *)(&pre_dentry);
    if( DENTRY_IS_LFN(LFN_ptr) )
       is_long = 1;         //target is LFN
    ...
    target->filename[namelen] = '\0';
      ...
      {
        strncpy(target->filename, name, 12);
        target->filename[12] = '\0';
	  }
#endif
    ...
    if (dir1 == dir2)
    {
        ...
        //target and old entry are both short name
#ifdef CYGPKG_FS_VFAT_NAME        
        if( namelen < 12 && is_long == 0 )
#endif
    ...
    else  //while target is LFN and write dentry to new location
    {
       if( disk->fat_type == FATFS_FAT32 && new_pos.cluster == 0 )
          new_pos.cluster = disk->fat_root_dir_cluster;
       
       while (true) //find the last entry in dir
       {
          err = read_raw_dentry(disk, &new_pos, &raw_dentry);
          if (err != ENOERR)
              diag_printf("error: in fatfs_rename_file function\n");

           //get the last entry in dir
          if (DENTRY_IS_ZERO(&raw_dentry))
              break;  

          new_pos.cluster_pos += DENTRY_SIZE;
            // Change cluster if needed
          if (new_pos.cluster_pos >= disk->cluster_size)
              err = get_next_cluster(disk, &new_pos, CO_EXTEND | CO_ERASE_NEW);
       }
      ...
      // write LFN into disk
       ...
       err = write_raw_LFN(disk, &new_pos, &LFN_entry);
       if (err != ENOERR)
           diag_printf("error: in fatfs_rename_file function\n");
       ...
       for( frg_cnt-- ; frg_cnt >= 0 ; frg_cnt-- )
         {
            ...
            if (new_pos.cluster_pos >= disk->cluster_size)
                err = get_next_cluster(disk, &new_pos, CO_NONE);
         }
    ...
    if( 1 == is_long )
      {
         ...
         while(true)
         {
           ...
           if ( DENTRY_IS_LFN(&pre_dentry) )
             {
                raw_dentry_set_deleted(disk, &raw_dentry);
                err = write_raw_dentry(disk, &old_pos, &raw_dentry);
                if (err != ENOERR)
                    return err;
             }
           else
...
//==============================================================================
#ifdef CYGPKG_FS_VFAT_NAME
int f32_read_next_raw_dentry(fatfs_disk_t *disk, fatfs_data_pos_t *pos, fat_raw_dir_entry_t *dentry)
{
	int err = ENOERR;
    ...
    while (true)
    {
        ...
        else
        {
            // Change cluster if needed
            if (pos->cluster_pos >= disk->cluster_size)
                err = get_next_cluster(disk, pos, 0x00);
        }

        if (err != ENOERR)
            break;

        err = read_raw_dentry(disk, pos, dentry); //check attr for long name struct
...
//-------------------------------------------------------------------------
void ascii2unicode(char *ascii_name, char *dest)
{
  int i,j;
    for( i=0,j=0 ; *(ascii_name+j)!='\0' ; i=i+2, j++ )
      *(dest+i) = *(ascii_name+j);
}
void byte_cpy(char *src, char *dest, int cnt)
{  
  int i;
    for( i=0 ; cnt>0 ; cnt--,i++ )
      *(dest+i) = *(src+i);
...
int fatfs_parser(fatfs_dir_entry_t *dentry, fatfs_disk_t *disk, struct vfat_descriptor *ventry)
{	
  vfat                 *vfat_ptr = NULL,*vptr = NULL;
  long_dentry_t        *long_de = NULL;
  fat_raw_dir_entry_t  raw_dentry;
  fatfs_data_pos_t     *pos = NULL;
  char                 long_name[510];
  int                  err=0,seqnum,final=0,error; //err count the list length
#ifdef VFAT_DEBUG_LEVEL
	char                 *next_name = NULL;
#endif
	
	pos = &(dentry->disk_pos);
	pos->cluster_pos += DENTRY_SIZE;
	memset(long_name, 32, 510);
	vfat_ptr = ventry;
	dentry_to_raw(dentry,&raw_dentry);
    ...
    long_de = (long_dentry_t *)(&raw_dentry);
	  ...
	  if( DENTRY_IS_LFN(long_de) )
	     {
         seqnum = (int)(long_de->id & ~0x40); //mask long name flag 0x40

//#ifdef VFAT_DEBUG_LEVEL
         unicode2ascii(long_de->name0_4  , &long_name[(seqnum-1)*13] ,   10 );
         unicode2ascii(long_de->name5_10 , &long_name[(seqnum-1)*13+5] , 12 );
         unicode2ascii(long_de->name11_12, &long_name[(seqnum-1)*13+11] , 4 );
//#endif         
/*       
         byte_cpy( long_de->name0_4,   (long_name+(seqnum-1)*26),    10);
         byte_cpy( long_de->name5_10,  (long_name+(seqnum-1)*26+10), 12);
         byte_cpy( long_de->name11_12, (long_name+(seqnum-1)*26+22) , 4);
*/
   			 if( ((long_de->id) & 0x40) == 0x40 )
   			 {
//#ifdef VFAT_DEBUG_LEVEL
   			    long_name[seqnum*13] = '\0';
//#endif
//            long_name[seqnum*26] = '\0';
         } 
...
#ifdef VFAT_DEBUG_LEVEL
        if( S_FATFS_ISDIR(raw_dentry.attr) )
				  diag_printf("<DIR>\t");
		    if( S_FATFS_ISARCHIVE(raw_dentry.attr) )
				  diag_printf("<FILE>\t");    	
        diag_printf("SIZE:%ld\t", (long)raw_dentry.size);
#endif
      	if( final )
      	{  
           byte_cpy( long_name , vptr->filename , 510 ); 
           final = 0;
        }
      	else
      	{   	
...
//#ifdef VFAT_DEBUG_LEVEL
//          next_name = (raw_dentry.name);
//				  diag_printf("%s", next_name);
//			    next_name = (raw_dentry.ext);
//				  diag_printf("%s", next_name);
//				  diag_printf("\n");
                
                int namelen = 0, extlen = 0;
                bool bContinue = 1;
                namelen = strlen(raw_dentry.name);               
				byte_cpy( raw_dentry.name, &long_name[0] , namelen);
				TRIM_RIGHT_STRING(bContinue, long_name, namelen);
              
                
                bContinue = 1;
                extlen = strlen(raw_dentry.ext); 
                byte_cpy( raw_dentry.ext, &vptr->ext_name[0] , extlen);              
				TRIM_RIGHT_STRING(bContinue, vptr->ext_name, extlen);
	
                vptr->ext_name[extlen] = '\0';


               	if (extlen > 0)
               	{
               	    long_name[namelen++] = '.';
			        byte_cpy( vptr->ext_name,  &long_name[namelen] ,extlen );
			        long_name[namelen + extlen] = '\0';
			    }

//#endif
/*          //8.3 file name
          ascii2unicode( raw_dentry.name, long_name );
          long_name[16] = '.';
          ascii2unicode( raw_dentry.ext, (long_name+18) );
          long_name[25] = '\0';*/
          byte_cpy( long_name , vptr->filename , 510 );
        }
            
        vptr->attr = raw_dentry.attr;
        vptr->size = raw_dentry.size;
        vptr->crt_time = raw_dentry.crt_time;
        vptr->crt_date = raw_dentry.crt_date;
        vptr->acc_date = raw_dentry.acc_date;
        vptr->next = NULL;
        memset(long_name, 32, 510);
		  }
 		error = f32_read_next_raw_dentry(disk, pos, &raw_dentry);
		//read dentry on disk, return raw dentry
		if (error != ENOERR)
		    return error;
 }while(1);
 return err;
