...
{
	cyg_uint32 wrt_addr=0, buf_addr=0, i=0, data=0;
	...
	iiow(0x90002700,0x00a50301);	//set resetj=1,osc_sel=1,cmppwdb=1,sel_hc_dev=1	
...
//Queue Head
	...
	iiow(0xBE010348,0x00003209);	//test
...
//while(1)
//{
//	diag_printf("status=(64) %08x;(68) %08x\n", iior(0xC0000064),iior(0xC0000068));
//	iiow(0x9000271c,0x00000001);	
//	diag_printf("status=(90002704) %08x\n", iior(0x90002704));
//	diag_printf("status=(90002700) %08x\n", iior(0x90002700));
//}
// ====== test
...
diag_printf("line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xC0000058)) ;	    
    iiow(0xC0000060,0x00000001);	//set configure flag
//    data = iior(0xC0000068);
//    data &= ~(1<<13);
diag_printf("line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
diag_printf("line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xC0000058)) ;	    
...
//    data = iior(0xC0000068);
//    data &= ~(1<<13);
diag_printf("line:%d status=[20]_%08X [64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000020),iior(0xC0000064),iior(0xC0000068)) ;
...
diag_printf("ehci_start:change port_owner: line:%d status=[20]_%08X [64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000020),iior(0xC0000064),iior(0xC0000068)) ;
diag_printf("line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xD0000058)) ;
	}
...
//#endif
	...
	if(data & 0x01)
	{
		...
		if(data & 0x02) 
			diag_printf("connect status changed\n");
		else
			diag_printf("connect status NO change\n");		
		
		//diag_printf("line%d data=%08X status=[64]_%08X [68]_%08X\n", __LINE__, data,iior(0xC0000064),iior(0xC0000068));
		//diag_printf("%d 2700=%08X\n", __LINE__,iior(0x90002700));		
		
		data &= ~(1<<13);	//clear port owner->SW writes 1 to this bit when the attached device is not HS. It means a companion HC owns.
		
		//  10, 11 bit: Line Status(RO)-> D+(11bit), D-(10bit)
		//Bits[11:10]	USB Status		Interpretation
		//00b			SE0				Not LS device, perform EHCI reset
		//10b			J-state			Not LS device, perform EHCI reset
		//01b			K-state			LS device, release ownership of port
		//11b			Undefined		Not LS device, perform EHCI reset 
		//-> This field is only valid when port enable bit(2b) is zero and current status bit(0b) is set to one.	

		i = (data >> 10) & 0x03;
		if(i != 0x01) 			//eliminate K-state(01b)
		{
			if(i==0x00)
				diag_printf("00_SE0\n");
			if(i==0x01)
...
cyg_uint8 ehci_check_portstatus(cyg_uint32 port)
{
	...
	//cyg_uint8 ret=IS_LOW_SPEED;	
	cyg_uint32 data=0;
	int i=0;

diag_printf("ehci_check_port:line:%d status=[20]_%08X [64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000020),iior(0xC0000064),iior(0xC0000068)) ;
diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xd0000054),iior(0xd0000058)) ;
	
	data = iior(0xC0000064+port*4);
	diag_printf("ehci_check_port: data=%08X status=[64]_%08X [20]_%08X\n", data,iior(0xC0000064),iior(0xC0000020));
///	data |= (1<<6);					//set force port resume
///	iiow(0xC0000064+port*4,data);
///	diag_printf("ehci_check_port: data=%08X status=[64]_%08X [68]_%08X\n", data,iior(0xC0000064),iior(0xC0000068));
...
diag_printf("    ehci_check_port: ret: %d\n", ret);
	return ret;
}
...
D(("ehci_control_transfer START: address: %d\n", address));
	if(address > 2)
	{
...
////diag_printf("[04]:%08X  [08]:%08X\n", iior(0xC0000004), iior(0xC0000008));
////diag_printf("[20]:%08X  [24]:%08X\n", iior(0xC0000020), iior(0xC0000024));
////diag_printf("[28]:%08X  [2C]:%08X\n", iior(0xC0000028), iior(0xC000002C));
////diag_printf("[34]:%08X  [38]:%08X\n", iior(0xC0000034), iior(0xC0000038));
////diag_printf("[60]:%08X  [64]:%08X  [68]:%08X\n", iior(0xC0000060), iior(0xC0000064), iior(0xC0000068));
//	diag_printf("line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
//	diag_printf("line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
...
//qHD
	...
	ret = EHCI_WaitComplete(__FUNCTION__, EHCI_DRAM_ADDR(QTD_BUF)+0x48, EHCI_TIMEOUT);
	if(ret== -1)
	{
		diag_printf("ehci_control: EHCI_waitComplete fail\n");
		for(i=0;i<8;i++)
		diag_printf("echi.c %02x\n", setup_data[i]);		
	}

	tmp  = iior(0xC0000020);
	tmp &= ~(1 << 5);	//Asynchronous Schedule disable
	iiow(0xC0000020, tmp);
	diag_printf("ehci.c: ehci_ctrl: AS disable\n");
...
//		diag_printf("address=%d > 2, ERROR!\n", address);
	...
	{
		diag_printf("ehci: bulk transfer: setup_data is NULL\n");
	}
...
//#endif
	...
	ret = EHCI_WaitComplete(__FUNCTION__, EHCI_DRAM_ADDR(QTD_BUF)+8, EHCI_TIMEOUT);
	if(ret == -1)
		diag_printf("ehci: ehci_commandTransfer: EHCI_WaitComplete fails\n");

	tmp  = iior(0xC0000020);
	tmp &= ~(1 << 5);	//Asynchronous Schedule disable
	iiow(0xC0000020, tmp);
	diag_printf("ehci.c: ehci_commandTransfer: AS disable\n");
...
//D(("EHCI_DataIn start\n"));
	...
	cyg_uint16 *pSrc16=NULL, *pTar16=NULL;
...
//QHD
	...
	tmp &= ~(1 << 5);	//Asynchronous Schedule disable
	iiow(0xC0000020, tmp);
	diag_printf("ehci.c: data_in: AS disable\n");
...
//QHD
	...
	tmp &= ~(1 << 5);	//Asynchronous Schedule disable
	iiow(0xC0000020, tmp);
	diag_printf("ehci.c: data_out: AS disable\n");
...
//QHD
	...
	tmp &= ~(1 << 5);	//Asynchronous Schedule disable
	iiow(0xC0000020, tmp);
	diag_printf("ehci.c: EHCI_BOT_Status: AS disable\n");
