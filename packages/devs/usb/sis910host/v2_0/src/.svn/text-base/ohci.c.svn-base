...
{
	cyg_uint32 wrt_addr=0, buf_addr=0, i=0;
//diag_printf("line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
	//HW USB RESEST - for no responses of second plug-in 
//	iiow(0x90004008,0x00000000);
//	iiow(0x90004008,0x00200000);
//	iiow(0x90004008,0x00000000);
//diag_printf("HW USB RESET\n");	
//diag_printf("line:%d status=[64]_%08X [20]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000020)) ;
//	test = iior(0xC0000064);
//	test |= (1<<7);
////	test |= (1<<0);
////	test |= (1<<1);
////	test |= (1<<2);
//	iiow(0xC0000064,test);
//diag_printf("line:%d test: %08X test1:%08X status=[64]_%08X [20]_%08X\n", __LINE__,test, test1, iior(0xC0000064),iior(0xC0000020)) ;	
//diag_printf("line:%d test: %08X test1:%08X status=[64]_%08X [20]_%08X\n", __LINE__,test, test1, iior(0xC0000064),iior(0xC0000020)) ;	
//	test1 = iior(0xC0000020);
//	test1 &= ~(1<<0);
//	iiow(0xC0000020,test1);
...
//diag_printf("line:%d test: %08X test1:%08X status=[64]_%08X [20]_%08X\n", __LINE__,test, test1, iior(0xC0000064),iior(0xC0000020)) ;	
diag_printf("line:%d status=[64]_%08X [20]_%08X\n", __LINE__, iior(0xC0000064),iior(0xC0000020)) ;	
diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xd0000054),iior(0xd0000058)) ;
...
//diag_printf("line=%d\n",__LINE__);
	...
	iiow(0xd0000034,11999);   //HcFmInterval
	//iiow(0xd0000034,0x04001000);   //HcFmInterval
//	iiow(0xd0000034,0x2edf1000);   //HcFmInterval,max=1024bits
...
//diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xC0000054),iior(0xC0000058)) ;
//diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xC0000054),iior(0xC0000058)) ;	
//		data = iior(0xC0000064);
//		data |= (1<<13);
//		iiow(0xC0000064+port*4, data);		
//diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xC0000054),iior(0xC0000058)) ;
//diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xC0000054),iior(0xC0000058)) ;		

diag_printf("ohci init END\n");	
diag_printf("line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xD0000058)) ;
	return 0;
}
...
cyg_uint32 ohci_start(cyg_uint32 port)
{
	...
	HD(("ohci: ohci_start\n"));
	//diag_printf("ohci_start: first: %d\n", first);
//	if(first) {
		//ohci_init();
//		first = 0;
//	}

diag_printf("ohci - line:%d status=[64]_%08X [68]_%08X\n", __LINE__,iior(0xC0000064),iior(0xC0000068)) ;
diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xD0000058)) ;	
		//switch port owner to ohci (1)
...
diag_printf("ohci - line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xD0000054),iior(0xD0000058)) ;		
	...
	iiow(0xd0000054+port*4,0x00000002);   //HcRhPortStatus[N]

	iiow(0xd0000054+port*4,1<<4);	//port reset	
	while(iior(0xd0000054+port*4) & (1<<4)) {
		if(!(iior(0xd0000054+port*4) & (1<<0))) {
			diag_printf("device disconnected on port %d!\n", port);
			return USB_OHCI_INIT_ERROR;
		}
	}
	
	status = iior(0xd0000054+port*4);
HD(("line:%d status=[54]_%08X [58]_%08X\n", __LINE__,iior(0xd0000054),iior(0xd0000058)));	
	if(iior(0xd0000054+port*4) & (1<<0)) {
HD(("device port %d reset complete!\n", port));
		if(status & (1<<9)) {
			diag_printf(" L O W   S P E E D   D E V I C E !!\n", port);
		}
	}
HD(("OHCI start FINISH!!\n"));
diag_printf("ohci_start: status: %x ; ret: %d\n", status, (!(status & (1<<9)) && (status & (1<<0))));
	if( ! (!(status & (1<<9)) && (status & (1<<0))) )
	{
		diag_printf("ohci_start: init error!\n");
		return USB_OHCI_INIT_ERROR;
	}
	else
	{
...
{
	cyg_uint8 ret=NO_DEIVCE;
	cyg_uint32 data=0;

	//iiow(0xC0000060,0x00000000);	//set configure flag	
	
	data = iior(0xD0000054+port*4);
	diag_printf("line%d data=%08X status=[54]_%08X [58]_%08X\n", __LINE__, data,iior(0xD0000054),iior(0xD0000058)) ;
	
	if(data & 0x01) 
	{
		HD(("device is present on port!\n"));
		
		if(data & 0x03)	//port enable status and current connect status
			ret = 0;
		else
			ret = USB_PORT_DISABLE;
			
		if(data & 0x10000)
			diag_printf("connect status change\n");
		else
			diag_printf("connect status NOT change\n");
	}

diag_printf("[04]:%08X  [08]:%08X\n", iior(0xd0000004), iior(0xd0000008));
diag_printf("[0C]:%08X  [10]:%08X\n", iior(0xd000000C), iior(0xd0000010));
diag_printf("[14]:%08X  [18]:%08X\n", iior(0xd0000014), iior(0xd0000018));
diag_printf("[1C]:%08X  [20]:%08X\n", iior(0xd000001C), iior(0xd0000020));
diag_printf("[24]:%08X  [28]:%08X\n", iior(0xd0000024), iior(0xd0000028));
diag_printf("[2C]:%08X  [30]:%08X\n", iior(0xd000002C), iior(0xd0000030));
diag_printf("[34]:%08X  [38]:%08X\n", iior(0xd0000034), iior(0xd0000038));
diag_printf("[3C]:%08X  [40]:%08X\n", iior(0xd000003C), iior(0xd0000040));
diag_printf("[44]:%08X  [48]:%08X\n", iior(0xd0000044), iior(0xd0000048));
diag_printf("[4C]:%08X  [50]:%08X\n", iior(0xd000004C), iior(0xd0000050));
diag_printf("[64]:%08X  [54]:%08X  [58]:%08X\n", iior(0xC0000064), iior(0xd0000054), iior(0xd0000058));
...
{
	cyg_uint32 wrt_addr=0, buf_addr=0, tmp=0;
	cyg_uint8 *ptr8=NULL;
//	cyg_uint8 *ptr8_print;
	...
	if(address > 2)
	{
		...
		return USB_OHCI_CTRL_PASSIN_ADDR_ERR;
	}

//debug
//diag_printf("OHCI: CONTROL package\n");
//for(i=0; i<8; i++, setup_data++)
//	diag_printf("%x", *setup_data);
//diag_printf("\n");
...
CD(("line=%d ED=%08X %08X %08X %08X\n", __LINE__, iior(wrt_addr),iior(wrt_addr+4),iior(wrt_addr+8),iior(wrt_addr+12)));
//cyg_uint32 xx=0;
	ret1 = OHCI_CheckTD(0,0x800,__FUNCTION__);

	ret2 = OHCI_CheckTD(1,0x800,__FUNCTION__);	

	ret3 = OHCI_CheckTD(2,0x800,__FUNCTION__);
	...
	if(ret1 || ret2 || ret3)
	{
		diag_printf("ohci_control_transfer:OHCI_CheckTD: ret: %d, %d, %d \n", ret1, ret2, ret3);
		return USB_OHCI_CTRL_CHECK_TD_FAIL;
	}
...
{
	cyg_uint32 rw_addr=0, tmp=0, no=0;
	cyg_uint8 *ptr8=NULL;
	cyg_uint32 ret=0;
	//cyg_uint8  count=0;
...
CD(("ohci.c: ohci_bulk_transfer: START\n"));
	...
	if(address > 2)
	{
		diag_printf("ohci: dev_idx=%d > 2, ERROR!\n", address);
		return USB_OHCI_BULK_PASSIN_ADDR_ERR;
	}
...
///		{
///			--count;
			ret = OHCI_CommandTransport(BOT_CMD, address);
///		}while(count && ret);
		
///		if(ret)
///		{
///			diag_printf("ohci_bulk_transfer: command transport fail: %d\n", ret);
///			return ret;
///		}
...
static cyg_uint32 OHCI_CheckTD(cyg_uint32 td_idx, cyg_uint32 wait, cyg_uint8 *src)
{
	cyg_uint32 read_addr=0, i=0, cc=0, ec=0;
...
//diag_printf("line=%d [0xD0000004]=%08X\n", __LINE__,iior(0xD0000004));
	...
	ret = OHCI_CheckTD(0,0x800,__FUNCTION__);
	OHCI_SkipED(ED_BULKOUT_1+dev_idx-1, 1);	//disable BULKOUT ED
...
static cyg_uint32 OHCI_DataIn(cyg_uint32 dev_idx, cyg_uint32 tar_addr, cyg_uint32 len)
{
	cyg_uint32 wrt_addr=0, buf_addr=0, tmp=0, i=0;
	cyg_uint8 *pSrc8=NULL, *pTar8=NULL;
	cyg_uint8 *pSrc16=NULL, *pTar16=NULL;
	cyg_uint32 *pSrc32=NULL, *pTar32=NULL;
	cyg_uint32 ret=0;
...
//diag_printf("#%d ED=%08X %08X %08X %08X\n", __LINE__, iior(wrt_addr),iior(wrt_addr+4),iior(wrt_addr+8),iior(wrt_addr+12));
	...
	ret = OHCI_CheckTD(0,len*16,__FUNCTION__);
	//wrt_addr = DRAM_ADDR(TD_BUF);
	//printT("line=%d ED=%08X %08X %08X %08X\n", __LINE__, iior(wrt_addr),iior(wrt_addr+4),iior(wrt_addr+8),iior(wrt_addr+12));
...
diag_printf("hc_init.c - usb_bulk_set_packetsize !!: %x\n", dev_address);
...
diag_printf("ohci.c: bulk init: bulk in\n");
//BULK OUT
