...
//	cyg_uint16 length = 0;	
	...
	cyg_uint32	tag=0;
	//cyg_uint8 	tmp1=0, tmp2=0, tmp3=0; tmp4=0;
...
if(data != NULL)
{
	diag_printf("mass.c - usb_bulk_status: command: %d\n", command);
	diag_printf(" data[0]: %x   data[1]: %x  \n", data[0], data[1]);
	diag_printf(" data[2]: %x   data[3]: %x  \n", data[2], data[3]);
	diag_printf(" data[4]: %x   data[5]: %x  \n", data[4], data[5]);
	diag_printf(" data[6]: %x   data[7]: %x  \n", data[6], data[7]);
}
	...
	{        
		...
		switch(command)
		{
			...
			case SC_READ_CAPACITY:	//Returned LBA and Block length in bytes -> no use yet!
				...
				//tmp_data = (data[3] | data[2]<<8 | data[1] <<16 | data[0]<<24);
				//diag_printf("mass.c: status: read_format temp data: %x\n", tmp_data);
				//data = (cyg_uint8 *)(&tmp_data);
				//diag_printf("mass.c: status: read_format temp data: %x\n", *data);
				break;
...
{//BOT only, according to SCSI spec
	...
	if(	(command == SC_TEST_UNIT_READY) || 
		(command == SC_INQUIRY) || 
		(command == SC_REQUEST_SENSE) ||
		(command == SC_READ_CAPACITY) )
	{
		lun = 0;
		pos = 0;
		blockNum=0;
	...
	{	
		...
		{		    		
			...
			if(result == CSW_STATUS_GOOD)
			{
				...
				diag_printf("mass.c - usb_mass_bulk_transport: %dth times: REQUEST SENSE OK, test unit again: %x\n", counter, udev[port].bLUN);
			}			
		}   
...
//	diag_printf("mass.c - usb_mass_bulk_transport: \n");
//	diag_printf(" data[0]: %x   data[1]: %x  \n", data[0], data[1]);
//	diag_printf(" data[2]: %x   data[3]: %x  \n", data[2], data[3]);
//	diag_printf(" data[4]: %x   data[5]: %x  \n", data[4], data[5]);
//	diag_printf(" data[6]: %x   data[7]: %x  \n", data[6], data[7]);
...
int usb_mass_bulk_cmd(cyg_uint8 command, struct USB_DEVICE *udev, struct hc_driver hcd, int direction, cyg_uint8 *data, cyg_uint32 data_len, cyg_uint32 data_pos)
{
	...
	port = get_usb_device();	
	diag_printf("mass.c: usb_bout_cmd: port: %d\n", port);
	if(USB_RW_DATA_ERROR == port)
	{
		diag_printf("mass.c: usb_bout_cmd: RW_DATA_ERROR port: %d\n");
		return USB_RW_DATA_ERROR;	
	}
	
	diag_printf("mass.c-usb_mass_bulk_transport: port: %d, bMaxLUN: %x, bLUN: %x\n",port,udev[port].bMaxLUN,  udev[port].bLUN);
   ...
   diag_printf("mass.c: usb_bout_cmd: after bulk init\n");
	//3. Check if the assigned current LUN is valid
	if(udev[port].bLUN > udev[port].bMaxLUN)
	{
		diag_printf("mass.c - current LUN is larger than MAX LUN: bLUN: %d; bMaxLUN: %d\n", udev[port].bLUN, udev[port].bMaxLUN);
		//udev[port].bLUN = USBMSC_DEFAULT_LUN_NUMBER;
		return USB_CHECK_CURRENT_LUN_ERROR;
	}
	
	diag_printf("mass.c: usb_bout_cmd: test unit\n");						
	//	Use Test_Unit_Ready to check if the current lun is valid
	counter = 3;				//20090119,retry counter
	do
	{	
...
//	diag_printf("mass.c - usb_mass_bulk_cmd: \n");
//	diag_printf(" data[0]: %x   data[1]: %x  \n", data[0], data[1]);
//	diag_printf(" data[2]: %x   data[3]: %x  \n", data[2], data[3]);
//	diag_printf(" data[4]: %x   data[5]: %x  \n", data[4], data[5]);
//	diag_printf(" data[6]: %x   data[7]: %x  \n", data[6], data[7]);
		...
		{		    		
			...
			{
				diag_printf("mass.c: SC_REQUEST_SENSE (after test_unit) ERROR\n");
			}			
		}   
	}while(result && counter);	

	diag_printf("mass.c: usb_bout_cmd: command: %d\n", command);	
	...
	switch(command)
	{
		case SC_READ_CAPACITY:
			...
			{
				//Prepare command package
				setup_len = usb_bulk_setup(	SC_READ_CAPACITY, READ, &cbw, SC_READ_CAPACITY_LENGTH, data_pos, udev[port].bMaxLUN, udev[port].bLUN);
				...
				usb_bulk_msg(	udev[port],					//USB_DEVICE
				                ...
				                setup_len, 
				                data, 
				                SC_READ_CAPACITY_LENGTH,	//0x08
				                (cyg_uint8 *)&csw);			//test unit ready only needs one byte
					                  		
				result = usb_bulk_status(SC_READ_CAPACITY, data, csw);		//check status value
				
				if(result != CSW_STATUS_GOOD)
				{
					diag_printf("mass.c: READ_CAPACITY error!!\n");	
					...
					if(result == CSW_STATUS_GOOD)
					{
						diag_printf("mass.c - usb_mass_bulk_transport: R-Capacity- REQUEST SENSE OK\n");
						result = 111;	
					}
					else
					{	
						diag_printf("mass.c: Request Fail on READ_CAPACITY error!!\n");					
						result = USB_DISK_READ_CAPACITY_ERROR;
					}
				}
			}while(111 == result);
			break;
		case SC_INQUIRY:
			...
			{
				setup_len = usb_bulk_setup(	SC_INQUIRY, direction, &cbw, SC_INQUIRY_LENGTH, data_pos, 0,0);
				result = usb_bulk_msg(	udev[port],		//USB_DEVICE
				         	       		hcd, 
				            	    	IN,  
				                		(cyg_uint8 *)&cbw, 
				                		setup_len, 
