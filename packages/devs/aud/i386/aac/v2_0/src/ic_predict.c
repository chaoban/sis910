...
static void flt_round(float *pf)
{
    ...
    tmp = *(unsigned int*)pf;
    flg = tmp & (unsigned int)0x00008000;
    tmp &= (unsigned int)0xffff0000;
    tmp1 = tmp;
    /* round 1/2 lsb toward infinity */
    if (flg)
    {
        tmp &= (unsigned int)0xff800000;       /* extract exponent and sign */
        tmp |= (unsigned int)0x00010000;       /* insert 1 lsb */
...
static short quant_pred(float x)
{
    short q;
...
static float inv_quant_pred(short q)
{
    float x;
    unsigned int *tmp = (unsigned int*)&x;
    *tmp = ((unsigned int)q)<<16;

    return x;
}

static void ic_predict(pred_state *state, real_t input, real_t *output, unsigned char pred)
{
    unsigned short tmp;
    short i, j;
    real_t dr1;
...
{
    unsigned char sfb, g, b;
    unsigned short i, offs, offs2;
...
void reset_all_predictors(pred_state *state, unsigned short frame_len)
{
    unsigned short i;
...
void ic_prediction(ic_stream *ics, real_t *spec, pred_state *state,
                   unsigned short frame_len, unsigned char sf_index)
{
    unsigned char sfb;
    unsigned short bin;
