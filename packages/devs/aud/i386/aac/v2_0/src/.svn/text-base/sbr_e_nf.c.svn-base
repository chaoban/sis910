...
void extract_envelope_data(sbr_info *sbr, unsigned char ch)
{
    ...
    for (l = 0; l < sbr->L_E[ch]; l++)
    {
        if (sbr->bs_df_env[ch][l] == 0)
        {
            ...
            unsigned char g = (l == 0) ? sbr->f_prev[ch] : sbr->f[ch][l-1];
            short E_prev;
...
/* calculates 1/(1+Q) */
/* [0..1] */
real_t calc_Q_div(sbr_info *sbr, unsigned char ch, unsigned char m, unsigned char l)
{
    if (sbr->bs_coupling)
    {
        /* left channel */
...
/* calculates Q/(1+Q) */
/* [0..1] */
real_t calc_Q_div2(sbr_info *sbr, unsigned char ch, unsigned char m, unsigned char l)
{
    if (sbr->bs_coupling)
    {
        if ((sbr->Q[0][m][l] < 0 || sbr->Q[0][m][l] > 30) ||
...
void envelope_noise_dequantisation(sbr_info *sbr, unsigned char ch)
{
    if (sbr->bs_coupling == 0)
    {
        short exp;
        unsigned char l, k;
        unsigned char amp = (sbr->amp_res[ch]) ? 0 : 1;
...
void unmap_envelope_noise(sbr_info *sbr)
{
    ...
    short exp0, exp1;
    unsigned char l, k;
    unsigned char amp0 = (sbr->amp_res[0]) ? 0 : 1;
    unsigned char amp1 = (sbr->amp_res[1]) ? 0 : 1;
