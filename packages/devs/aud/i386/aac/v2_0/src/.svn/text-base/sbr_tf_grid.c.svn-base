...
static unsigned char middleBorder(sbr_info *sbr, unsigned char ch);


/* function constructs new time border vector */
/* first build into temp vector to be able to use previous vector on error */
unsigned char envelope_time_border_vector(sbr_info *sbr, unsigned char ch)
{
    unsigned char l, border, temp;
    unsigned char t_E_temp[6] = {0};
...
void noise_floor_time_border_vector(sbr_info *sbr, unsigned char ch)
{
    ...
    if (sbr->L_E[ch] == 1)
    {
        ...
        unsigned char index = middleBorder(sbr, ch);
