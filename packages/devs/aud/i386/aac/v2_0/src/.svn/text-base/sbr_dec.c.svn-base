...
static unsigned char sbr_save_prev_data(sbr_info *sbr, unsigned char ch);
static void sbr_save_matrix(sbr_info *sbr, unsigned char ch);
...
{    
    ...
    if (id_aac == ID_CPE)
    {
        ...
        for (j = 0; j < 5; j++)
        {
            sbr.G_temp_prev[0][j] = malloc(64*sizeof(real_t));
            sbr.G_temp_prev[1][j] = malloc(64*sizeof(real_t));
            sbr.Q_temp_prev[0][j] = malloc(64*sizeof(real_t));
            sbr.Q_temp_prev[1][j] = malloc(64*sizeof(real_t));
        }
...
static unsigned char sbr_process_channel(sbr_info *sbr, real_t *channel_buf, qmf_t X[MAX_NTSR][64],
                                   unsigned char ch, unsigned char dont_process,
                                   const unsigned char downSampledSBR)
{
    short k, l;
    unsigned char ret = 0;
    ...
    if (sbr->Is_DRM_SBR)
    {
        sbr->bsco = max((int)sbr->maxAACLine*32/(int)sbr->frame_len - (int)sbr->kx, 0);
    } else {
#endif
        sbr->bsco = 0;
#ifdef DRM
...
//#define PRE_QMF_PRINT
    ...
    if ((sbr->just_seeked != 0) || dont_process)
    {
        ...
        for (l = 0; l < sbr->numTimeSlotsRate; l++)
        {
            unsigned char kx_band, M_band, bsco_band;
                             ...
                             const unsigned char just_seeked, const unsigned char downSampledSBR)
{
    ...
    sbr->ret += sbr_process_channel(sbr, channel, Y, 0, dont_process, downSampledSBR);
    /* subband synthesis */
    if (downSampledSBR)
    {
        sbr_qmf_synthesis_32(sbr, sbr->qmfs[0], Y, channel);
    } else {
        sbr_qmf_synthesis_64(sbr, sbr->qmfs[0], Y, channel);
    }
                               ...
                               const unsigned char just_seeked, const unsigned char downSampledSBR)
{
    ...
    for(i = 0; i < 32; i++)
   {
       for(j = 0; j < 32; j++)
       {
           X_left[i][j][0] = X_left[i][j][1] = 0;
           X_right[i][j][0] = X_right[i][j][1] = 0;
       }
   }
#else
    for(i = 0; i < 32; i++)
