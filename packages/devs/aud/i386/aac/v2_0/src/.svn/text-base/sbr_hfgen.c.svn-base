...
static void calc_prediction_coef(sbr_info *sbr, qmf_t Xlow[MAX_NTSRHFG][64],
                                 complex_t *alpha_0, complex_t *alpha_1, unsigned char k);
#endif
static void calc_chirp_factors(sbr_info *sbr, unsigned char ch);
                   ...
                   ,unsigned char ch)
{
    ...
    unsigned char offset = sbr->tHFAdj;
    unsigned char first = sbr->t_E[ch][0];
    unsigned char last = sbr->t_E[ch][sbr->L_E[ch]];
...
static void auto_correlation(sbr_info *sbr, acorr_coef *ac,
                             ...
                             unsigned char bd, unsigned char len)
{
...
static real_t mapNewBw(unsigned char invf_mode, unsigned char invf_mode_prev)
{
    switch (invf_mode)
    {
    case 1: /* LOW */
...
{
    unsigned char i, k;
    unsigned char odd, sb;
    unsigned char msb = sbr->k0;
    unsigned char usb = sbr->kx;
    unsigned char goalSbTab[] = { 21, 23, 32, 43, 46, 64, 85, 93, 128, 0, 0, 0 };
    /* (uint8_t)(2.048e6/sbr->sample_rate + 0.5); */
    unsigned char goalSb = goalSbTab[get_sr_index(sbr->sample_rate)];
