...
qmfa_info *qmfa_init(unsigned char channels)
{
    qmfa_info *qmfa = (qmfa_info*)malloc(sizeof(qmfa_info));

	/* x is implemented as double ringbuffer */
    qmfa->x = (real_t*)malloc(2 * channels * 10 * sizeof(real_t));
...
void sbr_qmf_analysis_32(sbr_info *sbr, qmfa_info *qmfa, const real_t *input,
                         qmf_t X[MAX_NTSRHFG][64], unsigned char offset, unsigned char kx)
{    
    unsigned int in = 0;
    unsigned char l;
    real_t u[64];
...
qmfs_info *qmfs_init(unsigned char channels)
{
    qmfs_info *qmfs = (qmfs_info*)malloc(sizeof(qmfs_info));

	/* v is a double ringbuffer */
    qmfs->v = (real_t*)malloc(2 * channels * 20 * sizeof(real_t));
