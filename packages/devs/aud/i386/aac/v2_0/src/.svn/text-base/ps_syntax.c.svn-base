...
/* type definitaions */
typedef const char (*ps_huff_tab)[2];

/* static data tables */
static const unsigned char nr_iid_par_tab[] = {
    10, 20, 34, 10, 20, 34, 0, 0
};
static const unsigned char nr_ipdopd_par_tab[] = {
    5, 11, 17, 5, 11, 17, 0, 0
};
static const unsigned char nr_icc_par_tab[] = {
    10, 20, 34, 10, 20, 34, 0, 0
};
static const unsigned char num_env_tab[][4] = {
    { 0, 1, 2, 4 },
    { 1, 2, 3, 4 }
};

/* binary lookup huffman tables */
static const char f_huff_iid_def[][2] = {
...
static const char t_huff_iid_def[][2] = {
    { /*0*/ -31, 1 },             /* index 0: 1 bits: x */
...
static const char f_huff_iid_fine[][2] = {
...
static const char t_huff_iid_fine[][2] = {
    { 1, /*0*/ -31 },             /* index 0: 1 bits: x */
...
static const char f_huff_icc[][2] = {
    { /*0*/ -31, 1 },            /* index 0: 1 bits: x */
...
static const char t_huff_icc[][2] = {
    { /*0*/ -31, 1 },            /* index 0: 1 bits: x */
    { /*1*/ -30, 2 },            /* index 1: 2 bits: 1x */
    { /*-1*/ -32, 3 },           /* index 2: 3 bits: 11x */
    { /*2*/ -29, 4 },            /* index 3: 4 bits: 111x */
...
static const char f_huff_ipd[][2] = {
    { 1, /*0*/ -31 },         /* index 0: 1 bits: x */
    { 2, 3 },                 /* index 1: 2 bits: 0x */
...
static const char t_huff_ipd[][2] = {
    { 1, /*0*/ -31 },         /* index 0: 1 bits: x */
    { 2, 3 },                 /* index 1: 2 bits: 0x */
...
static const char f_huff_opd[][2] = {
    { 1, /*0*/ -31 },         /* index 0: 1 bits: x */
    { 2, 3 },                 /* index 1: 2 bits: 0x */
...
static const char t_huff_opd[][2] = {
    { 1, /*0*/ -31 },         /* index 0: 1 bits: x */
    { 2, 3 },                 /* index 1: 2 bits: 0x */
    { 4, 5 },                 /* index 2: 3 bits: 00x */
    { /*1*/ -30, /*7*/ -24 }, /* index 3: 3 bits: 01x */
...
static unsigned short ps_extension(ps_info *ps, bitfile *ld,
                             const unsigned char ps_extension_id,
                             const unsigned short num_bits_left);
static void huff_data(bitfile *ld, const unsigned char dt, const unsigned char nr_par,
                      ps_huff_tab t_huff, ps_huff_tab f_huff, char *par);
static char ps_huff_dec(bitfile *ld, ps_huff_tab t_huff);


unsigned short ps_data(ps_info *ps, bitfile *ld, unsigned char *header)
{
    unsigned char tmp, n;
    unsigned short bits = (unsigned short)Get_processed_bits(ld);
    ...
    *header = 0;
    ...
    if (Get1bit(ld))
    {
        *header = 1;
        ...
        ps->enable_iid = (unsigned char)Get1bit(ld);

        if (ps->enable_iid)
        {
            ps->iid_mode = (unsigned char)Getbits(ld, 3);
        ...
        ps->enable_icc = (unsigned char)Get1bit(ld);

        if (ps->enable_icc)
        {
            ps->icc_mode = (unsigned char)Getbits(ld, 3);
        ...
        ps->enable_ext = (unsigned char)Get1bit(ld);
    }
    ...
    ps->frame_class = (unsigned char)Get1bit(ld);
    tmp = (unsigned char)Getbits(ld, 2);
    ...
    if (ps->frame_class)
    {
        ...
        {
            ps->border_position[n] = (unsigned char)Getbits(ld, 5) + 1;
        }
    }
    ...
    if (ps->enable_iid)
    {
        ...
        {
            ps->iid_dt[n] = (unsigned char)Get1bit(ld);
    ...
    if (ps->enable_icc)
    {
        for (n = 0; n < ps->num_env; n++)
        {
            ps->icc_dt[n] = (unsigned char)Get1bit(ld);
    ...
    if (ps->enable_ext)
    {
        ...
        {
            unsigned char ps_extension_id = (unsigned char)Getbits(ld, 2);

            num_bits_left -= 2;
            num_bits_left -= ps_extension(ps, ld, ps_extension_id, num_bits_left);
        }

        Getbits(ld, num_bits_left);
    }

    bits = (unsigned short)Get_processed_bits(ld) - bits;
                             ...
                             const unsigned short num_bits_left)
{
    ...
    {
        ps->enable_ipdopd = (unsigned char)Get1bit(ld);

        if (ps->enable_ipdopd)
        {
            for (n = 0; n < ps->num_env; n++)
            {
                ps->ipd_dt[n] = (unsigned char)Get1bit(ld);

                /* ipd_data */
                huff_data(ld, ps->ipd_dt[n], ps->nr_ipdopd_par, t_huff_ipd,
                    f_huff_ipd, ps->ipd_index[n]);

                ps->opd_dt[n] = (unsigned char)Get1bit(ld);
    ...
    bits = (unsigned char)Get_processed_bits(ld) - bits;
