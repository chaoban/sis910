...
static unsigned char estimate_current_envelope(sbr_info *sbr, sbr_hfadj_info *adj,
                                         qmf_t Xsbr[MAX_NTSRHFG][64], unsigned char ch);
static void calculate_gain(sbr_info *sbr, sbr_hfadj_info *adj, unsigned char ch);
#ifdef SBR_LOW_POWER
static void calc_gain_groups(sbr_info *sbr, sbr_hfadj_info *adj, real_t *deg, unsigned char ch);
static void aliasing_reduction(sbr_info *sbr, sbr_hfadj_info *adj, real_t *deg, unsigned char ch);
#endif
static void hf_assembly(sbr_info *sbr, sbr_hfadj_info *adj, qmf_t Xsbr[MAX_NTSRHFG][64], unsigned char ch);


sbr_hfadj_info adj;
unsigned char hf_adjustment(sbr_info *sbr, qmf_t Xsbr[MAX_NTSRHFG][64]
#ifdef SBR_LOW_POWER
...
static unsigned char get_S_mapped(sbr_info *sbr, unsigned char ch, unsigned char l, unsigned char current_band)
{
...
static unsigned char estimate_current_envelope(sbr_info *sbr, sbr_hfadj_info *adj,
                                         qmf_t Xsbr[MAX_NTSRHFG][64], unsigned char ch)
{
    unsigned char m, l, j, k, k_l, k_h, p;
    real_t nrg, div;
...
static real_t find_log2_Q(sbr_info *sbr, unsigned char k, unsigned char l, unsigned char ch)
{
    /* check for coupled energy/noise data */
    if (sbr->bs_coupling == 1)
    {
...
static real_t find_log2_Qplus1(sbr_info *sbr, unsigned char k, unsigned char l, unsigned char ch)
{
    /* check for coupled energy/noise data */
    if (sbr->bs_coupling == 1)
    {
...
static void calculate_gain(sbr_info *sbr, sbr_hfadj_info *adj, unsigned char ch)
{
    ...
    unsigned char current_t_noise_band = 0;
    unsigned char S_mapped;
    real_t G_boost;

    for (l = 0; l < sbr->L_E[ch]; l++)
    {
        unsigned char current_f_noise_band = 0;
        unsigned char current_res_band = 0;
        unsigned char current_res_band2 = 0;
        unsigned char current_hi_res_band = 0;
        ...
        for (k = 0; k < sbr->N_L[sbr->bs_limiter_bands]; k++)
        {
            ...
            real_t acc1 = 0;
            real_t acc2 = 0;
            unsigned char current_res_band_size = 0;
            unsigned char Q_M_size = 0;

            unsigned char ml1, ml2;
                        ...
                        qmf_t Xsbr[MAX_NTSRHFG][64], unsigned char ch)
{
    ...
    static char phi_im[] = { 0, 1, 0, -1 };
#endif
    static char phi_re[] = { 1, 0, -1, 0 };

    unsigned char m, l, i, n;
    unsigned short fIndexNoise = 0;
    unsigned char fIndexSine = 0;
    unsigned char assembly_reset = 0;
    ...
    for (l = 0; l < sbr->L_E[ch]; l++)
    {
        unsigned char no_noise = (l == sbr->l_A[ch] || l == sbr->prevEnvIsShort[ch]) ? 1 : 0;
        ...
        {
#ifdef SBR_LOW_POWER
            unsigned char i_min1, i_plus1;
            unsigned char sinusoids = 0;
#endif
            ...
            for (m = 0; m < sbr->M; m++)
            {
                ...
                if (h_SL != 0)
                {
                	unsigned char ri = sbr->GQ_ringbuf_index[ch];
                    for (n = 0; n <= 4; n++)
                    {
