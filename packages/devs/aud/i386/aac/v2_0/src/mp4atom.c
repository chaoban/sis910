...
static int mp4ff_atom_get_size(const char *data)
{
    unsigned int result;
...
static int mp4ff_atom_compare(const char a1, const char b1, const char c1, const char d1,
                                  const char a2, const char b2, const char c2, const char d2)
{
...
static unsigned char mp4ff_atom_name_to_type(const char a, const char b,
...
static int mp4ff_read_stsd(mp4ff_t *f)
{
    ...
    {
        unsigned long skip = mp4ff_position(f);
        unsigned long size;
        unsigned char atom_type = 0;
        size = mp4ff_atom_read_header(f, &atom_type, &header_size);
        skip += size;
...
static int mp4ff_read_mdhd(mp4ff_t *f)
{
    ...
    else //version == 0
    {
        ...
        f->track[f->total_tracks - 1]->duration = (temp == (unsigned int)(-1)) ? (unsigned long)(-1) : (unsigned long)(temp);
    }
    mp4ff_read_int16(f);
    mp4ff_read_int16(f);
    return 1;
...
#ifdef USE_TAGGING
static int mp4ff_read_meta(mp4ff_t *f, const unsigned long size)
{
    unsigned long subsize, sumsize = 0;
...
{
    unsigned long dest_position = mp4ff_position(f)+size-8;
