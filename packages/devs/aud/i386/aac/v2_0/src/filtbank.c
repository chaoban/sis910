...
#ifdef LD_DEC
    unsigned short frame_len_ld = frame_len/2;
#endif
...
static void imdct_long(fb_info *fb, real_t *in_data, real_t *out_data, unsigned short len)
{
#ifdef LD_DEC
    mdct_info *mdct = NULL;
...
#ifdef LTP_DEC
static void mdct(fb_info *fb, real_t *in_data, real_t *out_data, unsigned short len)
{
    mdct_info *mdct = NULL;
...
unsigned char ifilter_bank(fb_info *fb, unsigned char window_sequence, unsigned char window_shape,
                  unsigned char window_shape_prev, real_t *freq_in,
                  real_t *time_out, real_t *overlap,
                  unsigned char object_type, unsigned short frame_len)
{
    unsigned short i;
    real_t *transf_buf = (real_t *)malloc(2 * 1024 * sizeof(real_t));
    if(!transf_buf)
    {
        return 40;
    }
...
void filter_bank_ltp(fb_info *fb, unsigned char window_sequence, unsigned char window_shape,
                     unsigned char window_shape_prev, real_t *in_data, real_t *out_mdct,
                     unsigned char object_type, unsigned short frame_len)
{
    short i;
    real_t *windowed_buf = (real_t *)malloc(2 * 1024 * sizeof(real_t));
    if(!windowed_buf)
    {
        assert(0);
    }
    memset(windowed_buf, 0, 2 * 1024 * sizeof(real_t));
    const real_t *window_long = NULL;
    const real_t *window_long_prev = NULL;
    const real_t *window_short = NULL;
    const real_t *window_short_prev = NULL;
