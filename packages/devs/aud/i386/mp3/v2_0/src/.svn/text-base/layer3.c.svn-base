...
#ifdef AUD_TIME_ANALYSIS
#include <cyg/io/aud/time_test.h>		// for clock analysis
#endif
...
#define Hybrid_Dump(ptr, n)		do{	\
	{	\
			int i, j;	\
			printf("Hybrid Memory Dump:\n");		\
			for(i = 0; i < SBLIMIT; i++){		\
				for(j = 0; j < SSLIMIT; j++)		\
					printf("%f ", ptr[n][i][j]);			\
				printf("\n");					\
...
extern real pow1_1[2][16],pow2_1[2][16],pow1_2[2][16],pow2_2[2][16];
extern real tan1_1[16],tan2_1[16],tan1_2[16],tan2_2[16];
extern real aa_ca[8],aa_cs[8];
extern real COS9[9];
...
extern const struct bandInfoStruct bandInfo[9];
...
#ifdef AUD_TIME_ANALYSIS
static FILE *analysis = NULL;	
static unsigned long long start = 0, end = 0, frame_start = 0, frame_end = 0;
static unsigned long long scalor_tmp, 
							 huff_tmp, 
							 stereo_tmp, 
							 antialias_tmp, 
							 hybrid_tmp, 
							 synth_tmp;
static double scalor_cost = 0.0, 
		   huff_cost = 0.0, 
		   stereo_cost = 0.0, 
		   antialias_cost = 0.0, 
		   hybrid_cost = 0.0, 
...
extern int ABV_read(int size, uchar *dst);
...
*/
int 
decode_fromfile(AUD_tCtrlBlk *pAcb, short pcm_l[], short pcm_r[])
{
	static int flag = 0;
	int  ret = 0, len=0;
    	unsigned char buf[256];
	mp3data_struct *mp3data = (mp3data_struct *)pAcb->pPrivAttr;
	
	len = ABV_read(256, buf);
	/* first see if we still have data buffered in the decoder: */
    ret = lame_decode_header(buf, len, pcm_l, pcm_r, mp3data);
    if (ret != MP3_OK){
			if(ret == -1)
			EPRINTF(("IN %s LINE %d: %s\n", __FILE__, __LINE__, 
				"MP3_ERROR"));
			return ret;
    	}
    ...
    while (1) {
        ...
        if (len == 0) {
		    ...
		    ret = lame_decode_header(buf, len, pcm_l, pcm_r, mp3data);
		    if (ret<=0 && flag == 0) {		// Here is a hack by zhang sen
		    			flag ++;
					EPRINTF(("Release MP3DECODER Memory\n"));
   ...
   for (gr=0; gr<2; gr++) 
   {
     for (ch=0; ch<stereo; ch++) 
     {
       ...
       if(gr_infos->big_values > 288) {
          EPRINTF(("big_values too large! %i\n",gr_infos->big_values));
	  ...
	  /* window-switching flag == 1 for block_Type != 0 ... 
	   ...
	   unsigned int sbg = (getbits_fast(mp,3)<<3);
         ...
         if(gr_infos->block_type == 0) {
           EPRINTF(("Blocktype == 0 and window-switching == 1 not allowed.\n"));
   ...
   struct gr_info_s *gr_infos,int sfreq, int part2bits)
{
  ...
  /* MDH crash fix */
  {
      ...
      for (i = 0; i < 3; i++) {
	  if (l[i] < 0) {
	      EPRINTF(("mpg123: Bogus region length (%d)\n", l[i]));
	      l[i] = 0;
	  }
      }
  }
 ...
 */
  ...
  if(part2remain > 0)
    ...
    EPRINTF(("mpg123: Can't rewind stream by %d bits!\n",-part2remain));
    return 1; /* -> error */
  }
  return 0;
}
                ...
                int (*synth_1to1_ptr)(PMPSTR,real *,int,unsigned char *, int *) )
{
  ...
  { 
    ...
    if(stereo == 2) {
      ...
      long part2bits = 0;
      if(fr->lsf) {
        part2bits = III_get_scale_factors_2(mp,scalefacs[1],gr_infos,0);		
	  }
      else {
