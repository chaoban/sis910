                       ...
                       cyg_ucount32 *bytes_read )
{
    ...
    *bytes_read = 0;
    ...
    if (flags.buffering) {
        ...
        cyg_uint8 *buff_to_read_from = NULL;
        cyg_ucount32 bytes_available;
    
        bytes_available = io_buf.get_buffer_addr_to_read( (cyg_uint8 **)&buff_to_read_from );
        cyg_ucount32 count = (bytes_available < buffer_length) ? bytes_available : buffer_length;
...
#ifndef __jerry_stdio_performance_tuning
    if (!write_err)
    ...
    if (!flags.buffering) {
        ...
        *bytes_written = len;
        ...
        while ( buffer_length > 0 ) {
            ...
            // to write
            bytes_to_write=(bytes_available < buffer_length) ? bytes_available : buffer_length;
            // if we're line buffered, we may want want to flush if there's
            // a newline character, so lets find out
        
            if (flags.line_buffering) {
                for (newline_pos=0; newline_pos<bytes_to_write; newline_pos++) 
                {
