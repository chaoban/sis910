...
typedef struct vfat_descriptor
{
    char       filename[510];
	char       ext_name[4];
    cyg_uint16 crt_time;    //  Creation time
    cyg_uint16 crt_date;    //  Creation date
    cyg_uint16 acc_date;    //  Last access date  
    cyg_uint32 size;        //  Size of the file
    cyg_uint8  attr;     // Attribute bits for DOS compatability
    struct vfat_descriptor *next;
}vfat;
...
typedef int     cyg_fsop_setinfo  ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,
                                    int key, void *buf, int len );
typedef int     cyg_fsop_listdir ( cyg_mtab_entry *mte, cyg_dir dir, const char *name,vfat *ventry);
...
#define FSTAB_ENTRY( _l, _name, _data, _syncmode, _mount, _umount,      \
                     ...
                     _opendir, _chdir, _stat, _getinfo, _setinfo, _listdir)       \
...
__externC int mount( const char *devname,
                     ...
                     const char *fsname);
#ifdef SUPPORT_ALIGN_MOUNT
__externC int align_mount( const char *devname,
                     const char *dir,
                     const char *fsname,
                     cyg_uint32  align);
#endif
