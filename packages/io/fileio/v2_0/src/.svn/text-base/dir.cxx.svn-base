...
#include <cyg/infra/diag.h>           //added by jerry
#include <malloc.h>                    //need for free() 
//==========================================================================
...
//--------------------------------------------------------------------------
extern void close_listdir( struct vfat_descriptor *ventry )
{
   struct vfat_descriptor *ptr = NULL;
...
extern int listdir(const char *dirname, struct vfat_descriptor *ventry)
{
    ...
    int fd;
    cyg_file *file = NULL;
    cyg_mtab_entry *mte = cyg_cdir_mtab_entry; // current mount table
    cyg_dir dir = cyg_cdir_dir;// current dir
    ...
    if( 0 != ret )
    {
        cyg_fd_free(fd);
        cyg_file_free(file);
        diag_printf("ret=%d\tmtab_lookup fail\n",ret);
        return -1;
    }

	  LOCK_FS( mte );
	
    ret = mte->fs->listdir(mte,dir,name,ventry);
