...
{
//remove, R16 doesn't support it, and use fixed level mechanism.
...
//    case CYGNUM_HAL_INTERRUPT_TMU0_TUNI0 ... CYGNUM_HAL_ISR_MAX:            
    case CYGNUM_HAL_INTERRUPT_TMU0_TUNI0:            
        HAL_READ_UINT32(IO_IRQ_EN, temp);
        mask = 1 << (vector + 16);
        temp &= ~mask;             
        HAL_WRITE_UINT32(IO_IRQ_EN, temp);
