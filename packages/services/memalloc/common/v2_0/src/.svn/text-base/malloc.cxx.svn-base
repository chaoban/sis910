...
void *
malloc_align( size_t size, size_t align_size )
{
    void *data_ptr;

    CYG_REPORT_FUNCNAMETYPE( "malloc_align", "returning pointer %08x" );
    ...
    // ask the pool for the data
    data_ptr = POOL.try_alloc_align(size, align_size);
...
void diag_mallocinfo()
{
    struct mallinfo mem_info;
    
    mem_info = mallinfo();
    
    diag_printf("Heap Memory system: TotalSize=0x%08x TotalFree=0x%08x TotalAllocated=0x%08x MaxBlockSize=0x%08x\n ",
           mem_info.arena, mem_info.fordblks, mem_info.uordblks, mem_info.maxfree);
}

#endif  //__fila_malloc_align
