...
#include <cyg/hal/hal_misc.h>          //for global key
...
#ifdef __fila_malloc_align
//-----------------------------------
cyg_uint8 *
Cyg_Mempool_dlmalloc_Implementation::try_alloc_align(cyg_int32 bytes, cyg_int32 alignment)
{
  ...
  long      remainder_size;   /* its size */
  cyg_uint32 check_size_power2;
  
  /* The alignment argument must be a power of two */
  check_size_power2 = (cyg_uint32)alignment;
  if (check_size_power2 & (check_size_power2 - 1)) return 0;  /* power two failure*/
    
  /* If need less alignment than we give anyway, just relay to malloc */

  if (alignment <= (cyg_int32)MALLOC_ALIGNMENT) return try_alloc(bytes);

  /* Otherwise, ensure that it is at least a minimum chunk size */
  
  if (alignment <  (cyg_int32)MINSIZE) alignment = MINSIZE;

  /* Call malloc with worst case padding to hit alignment. */

  nb = request2size(bytes);
  m  = (char*)(try_alloc(nb + alignment + MINSIZE));
...
} // Cyg_Mempool_dlmalloc_Implementation::try_alloc_align()
#endif
