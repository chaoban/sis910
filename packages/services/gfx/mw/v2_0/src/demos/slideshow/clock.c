...
int
clock_main(int ac, char **av)
{
	GR_EVENT	event;		/* current event */
	GR_BITMAP	bitmap1fg[7];	/* bitmaps for first cursor */
	GR_BITMAP	bitmap1bg[7];
printf("clock before window create\n");
	if (GrOpen() < 0) {
	...
	w1 = GrNewWindowEx(
		...
		10, 50, CWIDTH, CHEIGHT, GrGetSysColor(GR_COLOR_WINDOW));
	printf("clock after window create\n");	
 ...
 */
void do_exposure1(GR_EVENT_EXPOSURE * ep){
	GR_COORD	midx = CWIDTH / 2;
	GR_COORD	midy = CHEIGHT / 2;
	int i, l;
printf("clock on exposure1\n");
