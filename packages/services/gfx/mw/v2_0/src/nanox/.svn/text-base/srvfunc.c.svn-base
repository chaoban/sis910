...
{
	GR_WINDOW	*wp = NULL;		/* window structure */
	GR_PIXMAP	*pp = NULL;
	GR_EVENT_CLIENT	*evp = NULL;		/* event-client structure */
...
{
	GR_WINDOW	*wp = NULL;		/* window structure */
	GR_WINDOW	*prevwp = NULL;	/* previous window pointer */
	GR_BOOL		overlap = NULL;	/* TRUE if there was overlap */
...
GrLowerWindow(GR_WINDOW_ID wid)
{
	GR_WINDOW	*wp = NULL;		/* window structure */
	GR_WINDOW	*prevwp = NULL;	/* previous window pointer */
	GR_WINDOW	*sibwp = NULL;		/* sibling window */
	GR_WINDOW	*expwp = NULL;		/* siblings being exposed */
...
{
	GR_WINDOW	*wp = NULL;		/* window structure */
	GR_WINDOW	*pwp = NULL;		/* parent window structure */
	GR_WINDOW	**mysibptr = NULL;	/* handle to my sibling ptr */
	GR_COORD	offx, offy;
...
{
	GR_GC		*gcp = NULL;		/* graphics context */
	GR_GC		*prevgcp = NULL;	/* previous graphics context */
...
{
	GR_GC		*oldgcp = NULL;	/* old graphics context */
	GR_GC		*gcp = NULL;		/* new graphics context */
	GR_GC_ID id;
...
{
	GR_REGION	*regionp = NULL;	/* region */
	GR_REGION	*prevregionp = NULL;	/* previous region */
...
{
	GR_REGION	*regionp = NULL;
	GR_REGION	*srcregionp1 = NULL;
	GR_REGION	*srcregionp2 = NULL;
...
{
	GR_REGION	*prgn1 = NULL;
	GR_REGION	*prgn2 = NULL;
	GR_BOOL result;
...
GrNewWindow(GR_WINDOW_ID parent, GR_COORD x, GR_COORD y, GR_SIZE width,
	...
	GR_COLOR bordercolor)
{
	GR_WINDOW	*pwp = NULL;		/* parent window */
	GR_WINDOW	*wp = NULL;		/* new window */
	GR_WINDOW_ID id;
...
{
	GR_CURSOR	*cursorp = NULL;
	GR_CURSOR	*prevcursorp = NULL;
...
{
	GR_WINDOW	*wp = NULL;
	GR_CURSOR	*cp = NULL;		/* cursor structure */
...
/* stretch image*/
void
GrStretchImageToFit(GR_IMAGE_ID imageid, GR_COORD x, GR_COORD y,
	GR_SIZE width, GR_SIZE height)
{
	
	SERVER_LOCK();

	GdStretchImageToFit(imageid, width, height );
...
EPRINTF("nano-X: skipping blit, sending expose event\n");
	...
	//pao-shu
	//printf("GrCopyArea width = %d, height =%d\n",width, height);
	if((op == MWROP_ROTATION_90)||(op==MWROP_ROTATION_270))
        GdRotateBlit(dp->psd, dp->x+x, dp->y+y, width, height, srcpsd, srcx, srcy,op);
    else	    
...
#if DYNAMICREGIONS
	GR_WINDOW *wp = NULL;
	GR_REGION *region = NULL;
	MWCLIPREGION *newregion = NULL;
...
{
	GR_GRABBED_KEY *keygrab = NULL;
	GR_GRABBED_KEY *last_keygrab = NULL;
...
{
	GR_GRABBED_KEY *keygrab = NULL;
	GR_GRABBED_KEY **keygrab_prev_next = NULL;
