...
{
	GR_EVENT_ERROR	*ep = NULL;		/* event to describe error */
...
{
	GR_EVENT_LIST	*elp = NULL;		/* current element list */
	GR_CLIENT	*oldcurclient = NULL;	/* old current client */
...
{
	MWKEY	 	mwkey = 0;		/* latest character */
	MWKEYMOD 	modifiers = 0;	/* latest modifiers */
	...
	if(keystatus < 0) {	    
		...
		return FALSE;
	} else if(keystatus > 0) {		/* Deliver events as appropriate: */	    
		switch (mwkey) {
		case MWKEY_QUIT:
			GsTerminate();
			/* no return*/
...
void GsDeliverButtonEvent(GR_EVENT_TYPE type, int buttons, int changebuttons,
			int modifiers)
{
	GR_EVENT_BUTTON	*ep = NULL;		/* mouse button event */
	GR_WINDOW	*wp = NULL;		/* current window */
	GR_EVENT_CLIENT	*ecp = NULL;		/* current event client */
	GR_CLIENT	*client = NULL;	/* current client */
...
{
	GR_EVENT_MOUSE	*ep = NULL;		/* mouse motion event */
	GR_WINDOW	*wp = NULL;		/* current window */
	GR_EVENT_CLIENT	*ecp = NULL;		/* current event client */
	GR_CLIENT	*client = NULL;	/* current client */
	GR_WINDOW_ID	subwid;		/* subwindow id event is for */
...
{
	GR_EVENT_KEYSTROKE	*ep = NULL;		/* keystroke event */
	GR_WINDOW		*wp = NULL;		/* current window */
	GR_WINDOW		*tempwp = NULL;	/* temporary window pointer */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
	GR_WINDOW_ID		subwid;		/* subwindow id event is for */
	GR_EVENT_MASK		eventmask;	/* event mask */
	GR_WINDOW		*kwp = NULL;
...
{
	GR_EVENT_EXPOSURE	*ep = NULL;		/* exposure event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
...
{
	GR_EVENT_LIST	*elp = NULL;		/* current element list */
	GR_EVENT_LIST	*prevelp = NULL;	/* previous element list */
	...
	GR_COORD y, GR_SIZE width, GR_SIZE height)
{
	...
	GR_EVENT_UPDATE		*ep = NULL;		/* update event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
...
{
	GR_EVENT_GENERAL	*gp = NULL;		/* general event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
	GR_EVENT_MASK		eventmask;	/* event mask */
...
{
	GR_WINDOW		*wp = NULL;
	GR_EVENT_GENERAL	*gp = NULL;
	GR_EVENT_CLIENT		*ecp = NULL;
...
{
	GR_EVENT_SCREENSAVER	*gp = NULL;		/* screensaver event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
...
{
	GR_EVENT_CLIENT_DATA_REQ *gp = NULL;		/* client data request event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
	GR_WINDOW *wp = NULL;
...
{
	GR_EVENT_CLIENT_DATA *gp = NULL;		/* client data request event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
	GR_WINDOW *wp;
...
{
	GR_EVENT_SELECTION_CHANGED *gp = NULL;		/* selection changed event */
	GR_EVENT_CLIENT		*ecp = NULL;		/* current event client */
	GR_WINDOW *wp = NULL;
...
{
	GR_EVENT_TIMER *event = NULL;	/* general event */
	GR_EVENT_CLIENT *ecp = NULL;	/* current event client */
	GR_WINDOW *wp = NULL;		/* current window */
