...
int UseHW;
/* 
    HwSwitch_910 = 0 -> Use SW 2D
    HwSwitch_910 = 1 -> Use HW 2D
*/
void GdSetHwUsage(int mode)
{
    printf("Set HW usage = %d\n",mode);
    UseHW = mode;
}
#endif
...
#define PIX2BYTES(n)	(((n)+7)/8)
	...
	if ((bpp == 32)
		...
		while (height > 0) {
			...
			trans = cr = rgborder
			    	? MWARGB(imagebits[3], imagebits[0], imagebits[1],
					imagebits[2])
			    	: MWARGB(imagebits[3], imagebits[2], imagebits[1],
					imagebits[0]);
				/*
 ...
 */
void
GdRotateBlit(PSD dstpsd, MWCOORD dstx, MWCOORD dsty, MWCOORD width, MWCOORD height,
	PSD srcpsd, MWCOORD srcx, MWCOORD srcy, long rop)
{
	int rx1, rx2, ry1, ry2;
	int px1, px2, py1, py2;
	...
	if(srcy+height > srcpsd->yvirtres)
		height = srcpsd->yvirtres - srcy;
	//pao-shu
    //printf("GdRotateBlit width = %d, height = %d\n", width,height);    
    if((rop == MWROP_ROTATION_90)||(rop == MWROP_ROTATION_270))
        nVisible = GdClipArea(dstpsd, dstx, dsty, dstx+height - 1, dsty+width-1);        
    else        
        nVisible = GdClipArea(dstpsd, dstx, dsty, dstx+width-1, dsty+height-1);
	...
	case CLIP_VISIBLE:
		...
		if (dstpsd != srcpsd)
			GdCheckCursor(srcpsd, srcx, srcy, srcx+width-1,
				srcy+height-1);
		//pao-shu
        //printf("GdRotateBlit Visible width = %d, height = %d\n", width,height);
		dstpsd->Blit(dstpsd, dstx, dsty, width, height,
			srcpsd, srcx, srcy, rop);
		GdFixCursor(dstpsd);
		if (dstpsd != srcpsd)
	...
	while(--count >= 0) {
		...
		if(pw > 0 && ph > 0) {
			...
			GdCheckCursor(dstpsd, srcx, srcy,
				srcx+width, srcy+height);
			//pao-shu
            //printf("GdRotateBlit while pw = %d, ph = %d\n", pw,ph);
			dstpsd->Blit(dstpsd, px1, py1, pw, ph, srcpsd,
				srcx + (px1-dstx), srcy + (py1-dsty), rop);
		}
		++prc;
