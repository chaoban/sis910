...
GdLoadImageFromFile(PSD psd, char *path, int flags)
{
  ...
  printf("GdLoadImageFromFile: open image: %s\n", path);
	...
	buffer = malloc_align(s.st_size, 1024);
#else
...
#endif
    //printf("GdLoadImageFromFile: open image 2: %s\n", path);
	GdImageBufferInit(&src, buffer, s.st_size);
	id = GdDecodeImage(psd, &src, path, flags);
    //pao-shu
    //printf("GdLoadImageFromFile: open image 3: %s id: %d\n", path, id);
#if HAVE_MMAP
...
GdDecodeImage(PSD psd, buffer_t * src, char *path, int flags)
{
	...
	if (loadOK == 0) 
		...
		//loadOK = GdDecodeJPEG(path, pimage, psd, flags);
#endif
...
#ifdef CYGPKG_HAL_R16 	        
	    if( (image2.imagebits = malloc_align(image2.pitch*height, 1024)) == NULL) {
#else	
		if( (image2.imagebits = malloc(image2.pitch*height)) == NULL) {
#endif		    
		    printf("image2.pitch = %d, height = %d\n",image2.pitch, height);
 ...
 */
void
GdStretchImageToFit(int id, MWCOORD width, MWCOORD height)
{
	PIMAGEITEM	pItem;
	PMWIMAGEHDR	pimage;
	...
	if (height != pimage->height || width != pimage->width) {
		...
		image2.width = pimage->width;
		image2.height = pimage->height;
		image2.planes = pimage->planes;
		image2.bpp = pimage->bpp;
		image2.pitch = pimage->pitch;
		image2.bytesperpixel = pimage->bytesperpixel;
		image2.compression = pimage->compression;
		image2.palsize = pimage->palsize;
		image2.palette = pimage->palette;	/* already allocated*/
		image2.transcolor = pimage->transcolor;
		image2.imagebits = pimage->imagebits;
		
		pimage->width = width;
		pimage->height = height;	
		GdComputeImagePitch(pimage->bpp, width, &(pimage->pitch),
			&(pimage->bytesperpixel));
			
		if( (pimage->imagebits = malloc(pimage->pitch*height)) == NULL) {
			EPRINTF("GdStretchImageToFit: no memory\n");
			return;
		}
		...
		/* Stretch full source to destination rectangle*/
		GdStretchImage(&image2, NULL, pimage, &rcDst);
...
GdFreeImage(int id)
{
	...
	if (pItem) {
		...
		if(pimage->imagebits)
		{		    		    
		    //pimage->imagebits = pimage->imagebits & 0x30000000;   		    
