...
{
	struct available_font *af = NULL, *best = NULL;
	char *family = NULL;
	int goodness;
...
{
	struct available_font *font = NULL;
	char fndry[128], *foundry = NULL;
...
{
	struct available_font *font = NULL, *next_font = NULL;
...
GdAddFont(char *fndry, char *fmly, char *fontname, PMWLOGFONT lf,
	unsigned int flags)
{
	struct available_font *font = NULL, *walk = NULL;
	int fontclass = 0;
...
int config_font(char *file, int line, int argc, char *argv[])
{
	...
	MWLOGFONT lf;
	char tmp[512];
	char *p = NULL, *q = NULL, *fndry = NULL, *family = NULL, *fontname = NULL;
	int size, t;
