...
#define SIS910_ROTATION_90          MWMODE_TO_ROP(MWMODE_MAX+1)
#define SIS910_ROTATION_270         MWMODE_TO_ROP(MWMODE_MAX+2)
#define SIS910_ROTATION_180         MWMODE_TO_ROP(MWMODE_MAX+3)
#define SIS910_ROTATION_HMIRROR     MWMODE_TO_ROP(MWMODE_MAX+4)
#define SIS910_ROTATION_VMIRROR     MWMODE_TO_ROP(MWMODE_MAX+5)
...
#ifdef SIS910_HW_2D
int RotationMode(int op)		
{						
	switch (op)                
	{
	    case MWROP_ROTATION_90     :
	        return 2;
	    case MWROP_ROTATION_270    :
	        return 3;
	    case MWROP_ROTATION_180    :
	        return 4;
	    case MWROP_ROTATION_HMIRROR:
	        return 5;
	    case MWROP_ROTATION_VMIRROR:
	        return 6;
	    default:
	        return 0;		
    }					       
...
int RopMode(int op)
{
	switch (op)                
	{
        case MWMODE_COPY		:/* src*/
            return ROPF_S;	
        case MWMODE_XOR		    :/* src ^ dst*/
            return ROPF_S_XOR_D;
        case MWMODE_OR		    :/* src | dst*/
            return ROPF_S_OR_D;
        case MWMODE_AND		    :/* src & dst*/
            return ROPF_S_AND_D;
        case MWMODE_CLEAR		:/* 0*/
            return ROPF_8_ZEROES;
        case MWMODE_SETTO1		:/* 11111111*/ /* obsolete name, will be MWMODE_SET*/
            return ROPF_8_ONES;
        case MWMODE_EQUIV		:/* ~(src ^ dst)*/
            return 0;
        case MWMODE_NOR		    :/* ~(src | dst) = ~src & ~dst*/
            return ROPF_NOT_S_AND_NOT_D;
        case MWMODE_NAND		:/* ~(src & dst) = ~src | ~dst*/
            return ROPF_NOT_S_OR_NOT_D;
        case MWMODE_INVERT		:/* ~dst*/
            return ROPF_NOT_D;
        case MWMODE_COPYINVERTED:/* ~src*/
            return ROPF_NOT_S;
        case MWMODE_ORINVERTED	:/* ~src | dst*/
            return ROPF_NOT_S_OR_D;
        case MWMODE_ANDINVERTED	:/* ~src & dst*/
            return ROPF_NOT_S_AND_D;
        case MWMODE_ORREVERSE	:/* src | ~dst*/
            return ROPF_S_OR_NOT_D;
        case MWMODE_ANDREVERSE	:/* src & ~dst*/
            return ROPF_S_AND_NOT_D;
        case MWMODE_NOOP		:/* dst*/	
            return ROPF_D;
        case MWMODE_XOR_FGBG	:/* src ^ background ^ dst (This is the Java XOR mode) */
            return 0;
	    default:
	        return 0;		
    }		
}
int AlphaBlendMode(int op)
{
	switch (op)                
	{
        case MWMODE_SRC	          :
            return ALPBLN_MODE_SRC;
        case MWMODE_DST	          :
            return 0;
        case MWMODE_SRC_OVER	  : 
            return ALPBLN_MODE_SRC_OVER;
        case MWMODE_DST_OVER	  : 
            return ALPBLN_MODE_DST_OVER;
        case MWMODE_SRC_IN	      :
            return ALPBLN_MODE_SRC_IN;
        case MWMODE_DST_IN	      :
            return ALPBLN_MODE_DST_IN;
        case MWMODE_SRC_OUT	      :
            return ALPBLN_MODE_SRC_OUT;
        case MWMODE_DST_OUT	      :
            return ALPBLN_MODE_DST_OUT;
        case MWMODE_SRC_ATOP	  : 
            return 0;
        case MWMODE_DST_ATOP	  : 
            return 0;
...
{
    //printf("fblinear32a_sis910 - linear32a_blit \n");
	ADDR8 dst8, src8;
    ...
    if(UseHW)
    {
        ...
        if (op == MWROP_COPY)
        {
            g2d_memmix(dst, src, ROPF_S, 0, w, h, slinelen, dlinelen);            
        }
        //else if( (op >= SIS910_ROTATION_90) && (op <= SIS910_ROTATION_VMIRROR))
        else if( (op >= MWROP_ROTATION_90) && (op <= MWROP_ROTATION_VMIRROR))
        {
            rotation = RotationMode(op);
            //printf("w = %d, h= %d, slinelen=%d, dlinelen =%d\n",w,h,slinelen,dlinelen);
            g2d_rotation(dst, src, 1, rotation, w, h, slinelen, dlinelen);
        }
        else if (MWROP_TO_MODE(op) <= MWMODE_SIMPLE_MAX) /* Last "simple" (non-alpha) mode */
        {
            rop = RopMode(op);
            g2d_memmix(dst, src, rop, 0, w, h, slinelen, dlinelen);
        }
        //else if (MWROP_TO_MODE(op) == MWMODE_DST_OVER)
        else if (MWROP_TO_MODE(op) == MWMODE_SRC_TRANSLUCENCY)
        {
            src8 = (ADDR8) src;
		    dst8 = (ADDR8) dst;
		    //printf("linear32a_blit MWMODE_SRC_TRANSLUCENCY\n");
		    while (h--) {
		    	for (i = w; --i >= 0;) {
		    		register int a;
		    		register int s;
        ...
        else
        {
            srcalpha = op & 0xff;
            dstalpha = (op >> 8) & 0xff;
            alphablend = AlphaBlendMode(MWROP_TO_MODE(op));
            
            g2d_alpha_blend(dst, src, alphablend, w, h, slinelen, dlinelen, 0 , 1, dstalpha, 1, srcalpha);
        }
		      ...
		      MWCOORD srcw, MWCOORD srch, long op)
{
    ...
    if(UseHW)
    {
        ...
        if( (srcw < 1920) && (srch < 1080) && (dstw < 1920) && (dsth < 1080))
        {
            //scaling ratio = i + n/m
            //p = submultiple(dsth, srch);
        	vsr_i = dsth / srch;
        	vsr_n = (dsth % srch) / p;
        	vsr_m = srch / p;
        	//p = submultiple(dstw, srcw);
        	hsr_i = dstw / srcw;
        	hsr_n = (dstw % srcw) / p;
        	hsr_m = srcw / p;
            g2d_memscl(0, dstpsd->addr, srcpsd->addr, srcx, srcy, dstx, dsty,
                       vsr_i, vsr_n, vsr_m, hsr_i, hsr_n, hsr_m, srcw, srch,slinelen, dlinelen);
        }
        else
        {
            h_scale = dstw / srcw;
            v_scale = dsth / srch;
            g2d_bilinear(src, srcw, srch, dst, h_scale, v_scale);
        }
    }
    else
#endif	
