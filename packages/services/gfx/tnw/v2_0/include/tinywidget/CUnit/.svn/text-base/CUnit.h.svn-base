...
#define CU_ASSERT_EQUAL_INT(actual, expected) \
    do{					      \
	CU_ASSERT_EQUAL(actual, expected);	\
	if(actual != expected){			\
	    fprintf(stderr,"In %s,line %d,expected value is %d,but actual is %d\n",__FILE__,__LINE__,expected,actual); \
	}   \
    }while(0);					


#define CU_ASSERT_EQUAL_STR(actual, expected) \
    do{					      \
	CU_ASSERT_STRING_EQUAL(actual,expected);	\
	if(strcmp(actual,expected)!=0){		\
	    fprintf(stderr,"In %s,line %d,expected value is \"%s\",but actual is \"%s\"\n",__FILE__,__LINE__,expected,actual); \
	}   \
    }while(0);					
